...
{
public :
    dirty_rect(lol::ivec2 aa, lol::ivec2 bb)
    {
        m_aa = aa;
        m_bb = bb;
        if (!(bb >= aa))
...
public:
    ...
    image_descriptor(lol::ivec2 size, int keep_dirties = 1, int static_memory = 0);
    ...
    void ClearDirties();
    void GetClip(lol::ivec2 &aa, lol::ivec2 &bb)
    {
        aa = m_aa; bb = m_bb;
    }
    void SetClip(lol::ivec2 aa, lol::ivec2 bb)
    {
        m_aa = Max(aa, lol::ivec2(0));
        m_bb = Min(Max(bb, m_aa + lol::ivec2(1)), m_size);
    }
    ...
    void ReduceDirties();
    void AddDirty(lol::ivec2 aa, lol::ivec2 bb);
    void DeleteDirty(lol::ivec2 aa, lol::ivec2 bb);
...
private:
    ...
    void MakePage(lol::ivec2 size, uint8_t *page_buffer);
    void DeletePage();
...
public:
    ...
    image(lol::ivec2 size, uint8_t *page_buffer = NULL, int create_descriptor = 0);
    ~image();

    void Lock();
    void Unlock();

    uint8_t Pixel(lol::ivec2 pos);
    void PutPixel(lol::ivec2 pos, uint8_t color);
    ...
    lol::ivec2 Size() const { return m_size; }

    void scroll(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
                int16_t xd, int16_t yd);
    void PutImage(image *screen, lol::ivec2 pos, int transparent = 0);
    void PutPart(image *screen, lol::ivec2 pos, lol::ivec2 aa, lol::ivec2 bb,
                 int transparent = 0);
    image *copy_part_dithered(int16_t x1, int16_t y1, int16_t x2, int16_t y2);
    void Bar(lol::ivec2 p1, lol::ivec2 p2, uint8_t color);
    void xor_bar(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint8_t color);
    void WidgetBar(lol::ivec2 p1, lol::ivec2 p2,
                   uint8_t light, uint8_t med, uint8_t dark);
    void Line(lol::ivec2 p1, lol::ivec2 p2, uint8_t color);
    void Rectangle(lol::ivec2 p1, lol::ivec2 p2, uint8_t color);
    void burn_led(int16_t x, int16_t y, int32_t num, int16_t color,
                  int16_t scale = 1);
    void SetClip(lol::ivec2 aa, lol::ivec2 bb);
    void GetClip(lol::ivec2 &aa, lol::ivec2 &bb);
    void InClip(lol::ivec2 aa, lol::ivec2 bb);
    void SetClip(int x1, int y1, int x2, int y2);
    void GetClip(int &x1, int &y1, int &x2, int &y2);
    ...
    void Scale(lol::ivec2 size);
    void SetSize(lol::ivec2 size, uint8_t *page = NULL);
